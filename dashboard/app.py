...
from utils import transform_backlog_to_summary, create_pivot,wrap_text,assign_tracks
...
import numpy as np
# Перенос длинных подписей оси Y (тегов групп)


#Делаем контент на всю страницу
st.set_page_config(
    page_title="Мой дашборд",
    layout="wide",
    initial_sidebar_state="expanded"
)
# Загрузка Excel-файла в боковой панели
...
# # 2) Для конкретных столбцов можно жестко фиксировать ширину или flex
...
column_task_name  = st.selectbox("Названия задач брать из колонки", df.columns.unique().to_list(), index=3)
...
with column1:
    ...
    if start_date > end_date:
        ...
        # 3) Опционально: выбор часов в день
        ...
        # 4) Вычисляем и выводим результат
...
with column2:
    st.write("В Q3 ", working_hours_between('2025-06-30', '2025-08-31', 8), ' рабочих дней')
    st.write("В Q4 ", working_hours_between('2025-06-30', '2025-08-31', 8), ' рабочих дней')




##############################################################################
# Раздел 2: Gantt-диаграмма
...
mask_ad = df_g['Роль'].isin(['Аналитик','UX/UI'])
...
mask_non_ad = ~df_g['Роль'].isin(['Аналитик','UX/UI','QA'])
df_g.loc[mask_non_ad, 'Start_calc'] = df_g.loc[mask_non_ad, 'AD_finish']
df_g.loc[mask_non_ad, 'Finish_calc'] = (
    df_g.loc[mask_non_ad, 'Start_calc']
    + df_g.loc[mask_non_ad, 'ТРЗ'].apply(BusinessDay)
)
...
df_g.loc[mask_qa, 'Finish_calc'] = (
    df_g.loc[mask_qa, 'Start_calc']
    + df_g.loc[mask_qa, 'ТРЗ'].apply(BusinessDay)
)
# st.write(df_g)
...
def plot_gantt(gantt_d,bar_mode = True):
    ...
    if bar_mode:
        ...
        y_group="Y_Group_lane"     # вместо "Y_Group"
    ...
    fig = px.timeline(
        ...
        text="Task",
        opacity=0.6,
        template='plotly',  # вернуть стандартные цвета
        )
    fig.update_traces(
        textposition="inside",
        ...
        width=0.9,               # чем больше — тем толще; подбирайте в вашем случае
        offsetgroup=0,
    )
    ...
    # 1. Корректный порядок категорий по оси Y
    y_cats = gantt_df[y_group].unique().tolist()
    fig.update_yaxes(
        categoryorder="array",
        categoryarray=y_cats,
        automargin=True,
        autorange="reversed"    # <- вот эта строка
    )
...
plot_gantt(gantt_df.sort_values(by=[sort_val_first,sort_val_second],),bar_mode)
