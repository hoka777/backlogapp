...
#         # constraintext="none",   # не пытаться впихнуть текст
#         # cliponaxis=False,        # позволить тексту выходить за бар
#         width=0.8,               # чем больше тем толще
#         offsetgroup=0,
#     )
...
#         # bargroupgap=1,           # убрать зазо“р между группами, если они есть
...
#     if not fit_names:
#         fig.update_layout(
#             uniformtext_minsize=font_size_names,
#             uniformtext_mode="show"
#         )
#         fig.update_traces(
#             constraintext="none",   # не пытаться впихнуть текст
...
#         # 1) убираем спринты, которые полностью раньше первой задачи
#         s = s[s['Дата окончания'] >= first_task_start].dropna(subset=['Дата начала','Дата окончания'])


#     # Фоновые полосы спринтов (только начиная с первой задачи)
#     if sprint_df is not None and len(sprint_df) > 0:
#         s = sprint_df.copy()
#         s['Дата начала'] = pd.to_datetime(s['Дата начала'], errors='coerce')
#         s['Дата окончания'] = pd.to_datetime(s['Дата окончания'], errors='coerce')
...
#         for _, sprint in s.iterrows():
#             sprint_start = sprint['Дата начала']
#             sprint_end   = sprint['Дата окончания']
#             sprint_num   = sprint['Номер спринта']

#             # 2) если спринт начался раньше первой задачи — подрезаем слева
#             x0 = max(sprint_start, first_task_start)
#             x1 = sprint_end
...
#             # аннотацию ставим по фактически рисуемому отрезку
#             mid = x0 + (x1 - x0) / 2
#             fig.add_annotation(
#                 x=mid, y=1.04, xref="x", yref="paper",
#                 text=f"Спринт {sprint_num}<br>{sprint_start:%d.%m.%Y}–{sprint_end:%d.%m.%Y}",
#                 showarrow=False, font=dict(size=font_size, color="Black"), align="center"
#             )
...
def plot_gantt(
    ...
    fig = add_date_markers(
        ...
        label_textangle=0      # вертикальная подпись
    )
    # вывод
    chart_html = fig.to_html(include_plotlyjs='cdn', full_html=False)
    components_html.html(
